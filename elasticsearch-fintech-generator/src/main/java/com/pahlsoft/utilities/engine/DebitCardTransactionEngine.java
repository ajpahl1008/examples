...
public class DebitCardTransactionEngine implements TransactionEngine {
    ...
    public DebitCardTransactionEngine() throws Exception {
        this.loadProperties();
    }
    ...
    public void run() {
        ...
        log.info("Starting Debit Card Transaction Engine " + Thread.currentThread().getId());
        ...
        while(true) {
            try {
                indexRequest = new IndexRequest("fintech-debit", "doc").source(TransactionGenerator.buildTransaction());
